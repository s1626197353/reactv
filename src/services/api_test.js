...
let url="http://sunmingfa.top:3335";
...
export const ajaxSignIn=async ({username,password})=>{
    ...
    return axios({url:`${url}/mock/signIn`,method:"post",data:{username,password}});
}
export const ajaxReg=async ({username,password})=>{
    // if(function(username))   密码判断
    return axios({url:`${url}/mock/reg`,method:"post",data:{username,password}});
}